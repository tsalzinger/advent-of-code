...
data class Path(
    ...
    fun extend(
        ...
        return Path(
            coordinates +
                (1..<steps).fold(listOf(coordinates.last()(direction))) { coordinates, _ ->
                    coordinates + coordinates.last()(direction)
                },
        )
    }

    override fun toString(): String = directions.toString()

    operator fun contains(coordinate: Grid2D.Coordinate): Boolean = coordinate in coordinates

    operator fun contains(coordinateWithDirection: Pair<Direction, Grid2D.Coordinate>): Boolean =
        coordinateWithDirection in directions.zip(coordinates.drop(1))
    ...
    override fun iterator(): Iterator<Grid2D.Coordinate> = coordinates.iterator()
}
